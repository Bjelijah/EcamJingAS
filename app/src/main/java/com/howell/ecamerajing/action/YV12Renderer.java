...
public class YV12Renderer implements Renderer,Constable {
	...
	public YV12Renderer(Context context, GLSurfaceView view,Handler handler) {
		...
		//		jni.nativeInit();
		CamJniUtil.YUVInit();
		CamJniUtil.YUVSetCallbackObject(this, 0);
		CamJniUtil.YUVSetCallbackMethodName(CALLBACK_SET_TIME, 0);
		CamJniUtil.YUVSetCallbackMethodName(CALLBACK_REQUEST_RENDER, 1);
	}
	...
	private void render(int i) {
		if (i==0){
			CamJniUtil.YUVRenderY();	
		}
		else if (i==1) {
			CamJniUtil.YUVRenderU();
		}else {
			CamJniUtil.YUVRenderV();
		}
	}
